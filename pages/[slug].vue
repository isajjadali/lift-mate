<template>
  <v-container align="center">
    <v-row justify="center">
      <v-col cols="12" sm="10">
        <v-card min-height="calc(100vh - 140px)" rounded="lg">
          <v-container class="pa-5">
            <v-row>
              <v-col v-if="page.title">
                ...
                <v-divider />
                <div class="pa-8" v-dompurify-html="page.content" />
              </v-col>
              <v-col v-else>
                <v-row justify="center"><h2>Page is not yet published!</h2></v-row>
              </v-col>
            </v-row>
          </v-container>
        </v-card>
...
export default {
  ...
  methods: {
    async updatePage() {
      this.page =
        this.$store.pages.find((page) => page.slug === this.slug) || {};
    },
  },
};
</script>
