<template>
  <v-container
    fluid
    class="static-page-editor"
  >
    <static-page-editor-loader v-if="isLoading" />
    <v-row v-else>
      <v-col
        cols="12"
        sm="4"
          ...
          <v-form v-model="valid">
            <v-container class="pa-8">
              <v-row>
                ...
                <v-col
                  :class="
                    $vuetify.display.xs ? 'text-center' : 'text-right'
                  "
              ...
              <v-row class="mt-0">
                <v-col>
                  <shared-custom-ck-editor
                    ...
                    :is-ckeditor-changed="isStaticPageChanged"
                    class="ckEditor"
                    @update:modelValue="isStaticPageChanged = false"
                  />
                </v-col>
              </v-row>
              <v-row class="mt-0">
...
<script>
  ...
  import StaticPageEditorLoader from '@/components/CustomLoader/StaticPageEditorLoader.vue';

  export default {
    name: 'StaticPageEditor',
    components: {
      StaticPageEditorLoader,
    },
    data: () => ({
      valid: false,
      isLoading: true,
    ...
    computed: {
      previewUrl() {
        return `${_.kebabCase(this.selectedPage.title)}`;
      },
        ...
        return (
          ...
          ['title', 'content'].every(
            (key) =>
              ...
              this.selectedPagePreviousCopy[key]
          )
        );
      },
    },
    ...
    methods: {
      ...
      onPageChange(page, index) {
        this.selectedPage = { ...page };
        this.selectedPagePreviousCopy = _.cloneDeep(
          this.pages[index]
        );
        this.isStaticPageChanged = true;
      },
    },
  };
</script>
